...
use Modules\RestaurantWebAddon\App\Exports\ExpenseExport;

class AcnooExpenseController extends Controller
{
    public function __construct()
    {
        $this->middleware('check.permission:expense.view')->only('index');
        $this->middleware('check.permission:expense.create')->only('store');
        $this->middleware('check.permission:expense.update')->only('update', 'status');
        $this->middleware('check.permission:expense.delete')->only('destroy', 'deleteAll');
    }

    public function index()
    {
        $expense_categories = ExpenseCategory::where('business_id', auth()->user()->business_id)->whereStatus(1)->latest()->get();
        $expenses = Expense::with('category:id,categoryName', 'payment_type:id,name')->where('business_id', auth()->user()->business_id)->latest()->paginate(20);
        $payment_types = PaymentType::where('business_id', auth()->user()->business_id)->whereStatus(1)->latest()->get();

        return view('restaurantwebaddon::expenses.index', compact('expenses', 'expense_categories', 'payment_types'));
    }
    ...
    public function acnooFilter(Request $request)
    {
            ...
            ->when(request('search'), function ($q) use ($request) {
                $q->where(function ($q) use ($request) {
                    $q->where('amount', 'like', '%' . $request->search . '%')
                        ...
                        ->orWhere('expenseDate', 'like', '%' . $request->search . '%')
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('restaurantwebaddon::expenses.datas', compact('expenses'))->render()
            ]);
        }
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $expense = Expense::create($request->except('user_id', 'business_id') + [
                'user_id' => auth()->id(),
                'business_id' => auth()->user()->business_id,
            ]);
    ...
    public function generatePDF(Request $request)
    {
        ...
        $pdf = Pdf::loadView('restaurantwebaddon::expenses.pdf', compact('expenses'));
        return $pdf->download('expense-list.pdf');
    }

    public function exportExcel()
    {
        return Excel::download(new ExpenseExport, 'expense-list.xlsx');
    }

    public function exportCsv()
    {
        return Excel::download(new ExpenseExport, 'expense-list.csv');
    }
}
