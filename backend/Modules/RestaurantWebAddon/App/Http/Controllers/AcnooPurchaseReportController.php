...
use Modules\RestaurantWebAddon\App\Exports\PurchaseReportExport;

class AcnooPurchaseReportController extends Controller
{
    public function __construct()
    {
        $this->middleware('check.permission:purchaseReport.view')->only('index');
    }
    ...
    public function index()
    {
        ...
        return view('restaurantwebaddon::reports.purchase.purchase-reports', compact('purchases'));
    }

    public function acnooFilter(Request $request)
    {
        $businessId = auth()->user()->business_id;
        $purchasesQuery = Purchase::with('party:id,name', 'payment_type:id,name')
            ->where('business_id', $businessId);
        ...
        $purchases = $purchasesQuery->latest()->paginate($request->per_page ?? 10);

        if ($request->ajax()) {
            return response()->json([
                'data' => view('restaurantwebaddon::reports.purchase.datas', compact('purchases'))->render()
            ]);
        }
    ...
    public function generatePDF(Request $request)
    {
        ...
        $pdf = Pdf::loadView('restaurantwebaddon::reports.purchase.pdf', compact('purchases'));
        return $pdf->download('purchase-report.pdf');
    }

    public function exportExcel()
    {
        return Excel::download(new PurchaseReportExport, 'purchase-reports.xlsx');
    }

    public function exportCsv()
    {
        return Excel::download(new PurchaseReportExport, 'purchase-reports.csv');
    }
}
