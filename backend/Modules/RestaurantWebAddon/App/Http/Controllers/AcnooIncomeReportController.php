...
use Modules\RestaurantWebAddon\App\Exports\IncomeReportExport;

class AcnooIncomeReportController extends Controller
{
    public function __construct()
    {
        $this->middleware('check.permission:incomeReport.view')->only('index');
    }
    ...
    public function index()
    {
        ...
        return view('restaurantwebaddon::reports.income.income-reports', compact('income_reports'));
    }
    ...
    public function acnooFilter(Request $request)
    {
        ...
        $income_reports = $incomeQuery->latest()->paginate($request->per_page ?? 10);

        $total_income = $incomeQuery->sum('amount');

        if ($request->ajax()) {
            return response()->json([
                'data' => view('restaurantwebaddon::reports.income.datas', compact('income_reports'))->render(),
    ...
    public function generatePDF(Request $request)
    {
        ...
        $pdf = Pdf::loadView('restaurantwebaddon::reports.income.pdf', compact('income_reports'));
        return $pdf->download('income-report.pdf');
    }

    public function exportExcel()
    {
        return Excel::download(new IncomeReportExport, 'income-reports.xlsx');
    }

    public function exportCsv()
    {
        return Excel::download(new IncomeReportExport, 'income-reports.csv');
    }
}
