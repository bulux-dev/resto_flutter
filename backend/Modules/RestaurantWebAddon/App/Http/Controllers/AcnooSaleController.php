...
class AcnooSaleController extends Controller
{
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            // If this sale has a KOT ticket, update its table number and booking status
            if ($sale->kot_id) {
                $kot = KotTicket::find($sale->kot_id);
                if ($kot) {
                    // If kot had a previous table, unbook it when changed or removed
                    if ($kot->table_id && ($request->table_id != $kot->table_id)) {
                        Table::where('id', $kot->table_id)->update(['is_booked' => false]);
                    }

                    // If new table is provided and sale is dine_in, book it
                    if ($request->table_id && $request->sales_type == 'dine_in') {
                        Table::where('id', $request->table_id)->update(['is_booked' => true]);
                    }
