...
use Modules\RestaurantWebAddon\App\Exports\DueExport;

class AcnooDueController extends Controller
{
    public function __construct()
    {
        $this->middleware('check.permission:dueCollection.view')->only('index', 'walk_dues');
        $this->middleware('check.permission:dueCollection.create')->only('collectDue', 'collectDueStore');
    }
    ...
    public function index()
    {
        ...
        return view('restaurantwebaddon::dues.index', compact('dues', 'total_supplier_due', 'total_customer_due'));
    }
    ...
    public function acnooFilter(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('restaurantwebaddon::dues.datas', compact('dues'))->render()
            ]);
        }
    ...
    {
        $walk_in_customers = Sale::with('dueCollect')
            ->where('business_id', auth()->user()->business_id)
            ->whereNull('party_id')
            ->where('dueAmount', '>', 0)
            ->latest()
        ...
        return view('restaurantwebaddon::walk-dues.index', compact('walk_in_customers'));
    }
    ...
    public function walk_dues_filter(Request $request)
    {
        ...
        if ($request->ajax()) {
            return response()->json([
                'data' => view('restaurantwebaddon::walk-dues.datas', compact('walk_in_customers'))->render()
            ]);
        }
        return redirect(url()->previous());
    }

    public function collectDue(Request $request, string $id)
    {
        ...
        if ($request->source === 'walk-in') {
            ...
            return view('restaurantwebaddon::dues.collect-due', compact('walk_due', 'payment_types', 'is_walk_in'));
        }
        ...
        return view('restaurantwebaddon::dues.collect-due', compact('party', 'payment_types', 'is_walk_in'));
    }
    ...
    public function getInvoice(string $id)
    {
        ...
        return view('restaurantwebaddon::dues.invoice', compact('due_collect', 'party'));
    }
    ...
    public function walkDuesGetInvoice(string $id)
    {
        ...
        return view('restaurantwebaddon::walk-dues.invoice', compact('due_collect', 'walk_in_customer'));
    }
    ...
    public function generatePDF()
    {
        ...
        $pdf = Pdf::loadView('restaurantwebaddon::dues.pdf', compact('dues'));
        return $pdf->download('due-list.pdf');
    }

    public function exportExcel()
    {
        return Excel::download(new DueExport, 'due-list.xlsx');
    }

    public function exportCsv()
    {
        return Excel::download(new DueExport, 'due-list.csv');
    }
}
