...
use Modules\RestaurantWebAddon\App\Exports\ExpenseReportExport;

class AcnooExpenseReportController extends Controller
{
    public function __construct()
    {
        $this->middleware('check.permission:expenseReport.view')->only('index');
    }
    ...
    public function index()
    {
        ...
        return view('restaurantwebaddon::reports.expense.expense-reports', compact('expense_reports'));
    }
    ...
    public function acnooFilter(Request $request)
    {
        ...
        $expense_reports = $expenseQuery->latest()->paginate($request->per_page ?? 10);

        if ($request->ajax()) {
            return response()->json([
                'data' => view('restaurantwebaddon::reports.expense.datas', compact('expense_reports'))->render(),
    ...
    public function generatePDF(Request $request)
    {
        ...
        $pdf = Pdf::loadView('restaurantwebaddon::reports.expense.pdf', compact('expense_reports'));
        return $pdf->download('expense-reports.pdf');
    }

    public function exportExcel()
    {
        return Excel::download(new ExpenseReportExport, 'expense-reports.xlsx');
    }

    public function exportCsv()
    {
        return Excel::download(new ExpenseReportExport, 'expense-reports.csv');
    }
}
