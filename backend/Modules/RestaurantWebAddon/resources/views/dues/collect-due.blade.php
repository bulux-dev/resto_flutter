    ...
    <div class="container-fluid">
        <div class="section-title mb-3">
            <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                <span> <a href="{{ route('business.dues.index') }}">/ {{ __('Due List') }}</a></span>
                <span>/ @if($is_walk_in)
                        {{ __('Collect Due') }}
                    @else
                        {{ $party->type == "supplier" ? __('Pay Due') : __('Collect Due') }}
                            ...
                            <div class="row">
                                <div class="col-lg-6 mb-2">
                                    ...
                                    <select id="invoiceSelect" name="invoiceNumber" class="form-control table-select w-100">
                                        @if(!$is_walk_in)
                                        <option  data-opening-due="{{ $party->opening_balance }}" value="">{{ __('Select an Invoice') }}</option>
                                ...
                                <div class="col-lg-6 mb-2">
                                    ...
                                    <input type="number" id="totalAmount" value="{{ !$is_walk_in ? ($party->opening_balance ?? 0) : $walk_due->dueAmount }}" readonly class="form-control">
                                </div>
                                ...
                                <div class="col-lg-6 mb-2">
                                    ...
                                    <input type="number" id="dueAmount" value="{{ !$is_walk_in ? ($party->opening_balance ?? 0) : $walk_due->dueAmount }}" readonly class="form-control">
                                </div>
