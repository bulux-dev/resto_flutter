...
@section('main_content')
    <div class="erp-table-section">
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                    <span>/ {{ __('VAT List') }}</span>
                </h2>

                <form action="{{ route('business.vats.filter') }}" method="post" class="filter-form" table="#vats-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.vats.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.vats.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.vats.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('vat.create')
                            <a href="#vat-add-modal" class="theme-btn print-btn text-light active custom-a"
                                data-bs-toggle="modal"> <i class="fas fa-plus-circle me-1"></i> {{ __('Add Vat') }}</a>
                        @endusercan
            ...
            <div class="card shadow-sm vatlist-body min-h-0">

                    @usercan('vat.delete')
                        <div class="delete-item delete-show multi-delete-container d-none">
                ...
                <div class="responsive-table min-h-0 vatlist-body mt-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="vats-data">
                            @include('restaurantwebaddon::vats.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
        ...
        @include('restaurantwebaddon::vat-groups.index')
    </div>
@endsection

@push('modal')
    @include('restaurantwebaddon::component.delete-modal')
    @include('restaurantwebaddon::vats.create')
    @include('restaurantwebaddon::vats.edit')
@endpush
