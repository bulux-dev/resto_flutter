@foreach ($staffs as $staff)
    <tr>
        @usercan('staff.delete')
        <td class="w-60 checkbox">
            <input type="checkbox" name="ids[]" class="delete-checkbox-item  multi-delete" value="{{ $staff->id }}">
        </td>
        @endusercan
        ...
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    @usercan('staff.view')
                    <a title="{{__('View')}}" data-bs-placement="top" href="#staff-view" class="staff-view-btn action-btn view" data-bs-toggle="modal"
                        data-name="{{ $staff->name }}" data-email="{{ $staff->email }}"
                        data-phone="{{ $staff->phone }}" data-designation="{{ $staff->designation == 'delivery_boy' ? __('Delivery Boy') : ucfirst($staff->designation) }}"
                    ...
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="#staff-edit-modal" class="staff-edit-btn action-btn edit" data-bs-toggle="modal"
                        data-url="{{ route('business.staffs.update', $staff->id) }}"
                        data-name="{{ $staff->name }}" data-email="{{ $staff->email }}"
                        data-phone="{{ $staff->phone }}" data-designation="{{ $staff->designation }}"
                        data-address="{{ $staff->address }}">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                    ...
                    @endusercan
                    @usercan('staff.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.staffs.destroy', $staff->id) }}"
