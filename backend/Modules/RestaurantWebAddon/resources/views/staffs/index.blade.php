    ...
    <div class="erp-table-section">
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                ...
                <form action="{{ route('business.staffs.filter') }}" method="post" class="filter-form" table="#staffs-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        {{-- Filter dropdown --}}
                        <div class="select-wrapper filter-dropdown">
                            <select name="designation" class="custom-select lg-filter">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.staffs.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.staffs.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.staffs.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('staff.create')
                            <a type="button" href="#staff-create-modal" class="add-order-btn rounded-2" class="btn btn-primary"
                                data-bs-toggle="modal"><i class="fas fa-plus-circle me-1"></i>{{ __('Add Staff') }}
                            </a>
                        @endusercan
                    </div>
                </form>
                        ...
                        <div class="delete-item delete-show multi-delete-container d-none mb-3">
                            <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                                ...
                                <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal"
                                    data-url="{{ route('business.staffs.delete-all') }}">{{ __('Delete') }}</button>
                            </div>
                        </div>
                    @endusercan
                </div>
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="staffs-data">
                            @include('restaurantwebaddon::staffs.datas')
                        </tbody>
                    </table>
                </div>
                <nav>
                    <ul class="pagination">
                        <li class="page-item">{{ $staffs->links('vendor.pagination.bootstrap-5') }}</li>
                    </ul>
                </nav>
            </div>
        </div>
...
@push('modal')
    ...
    @include('restaurantwebaddon::staffs.create')
    @include('restaurantwebaddon::staffs.edit')
    @include('restaurantwebaddon::staffs.view')
@endpush
