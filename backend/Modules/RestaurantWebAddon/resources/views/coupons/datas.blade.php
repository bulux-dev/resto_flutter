@foreach ($coupons as $coupon)
    <tr>
        @usercan('coupon.delete')
        <td class="w-60 checkbox">
            <input type="checkbox" name="ids[]" class="delete-checkbox-item  multi-delete" value="{{ $coupon->id }}">
        </td>
        @endusercan
        <td>{{ ($coupons->currentPage() - 1) * $coupons->perPage() + $loop->iteration }}</td>
        <td class="text-start">
            <img class="table-img" src="{{ asset($coupon->image ?? 'assets/img/icon/no-image.svg') }}" alt="coupon-img">
        </td>
        ...
        <td class="text-start">{{ formatted_date($coupon->start_date) }}</td>
        <td class="text-start">{{ formatted_date($coupon->end_date) }}</td>
        <td class="text-center">
            @if (\Carbon\Carbon::parse($coupon->end_date)->isPast())
                <span class="expired-status">{{ __('Expired') }}</span>
            @elseif (\Carbon\Carbon::parse($coupon->start_date)->isFuture())
                <span class="upcoming-status">{{ __('Upcoming') }}</span>
            @else
                <span class="available-status">{{ __('Available') }}</span>
            @endif
        </td>
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    @usercan('coupon.view')
                    <a title="{{__('View')}}" data-bs-placement="top" href="#coupon-view" class="coupon-view-btn action-btn view" data-bs-toggle="modal"
                        data-image="{{ asset($coupon->image ?? 'assets/img/icon/no-image.svg') }}"
                        data-name="{{ $coupon->name }}" data-code="{{ $coupon->code }}"
                        data-discount="{{ $coupon->discount_type == 'percentage' ? $coupon->discount.'%' : currency_format($coupon->discount) }}"
                        data-start-date="{{ $coupon->start_date }}" data-end-date="{{ $coupon->end_date }}"
                        data-desc="{{ $coupon->description }}">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <g clip-path="url(#clip0_4004_55660)">
                                <path
                                    d="M12.5 5C16.6818 5 20.264 9.01321 21.7572 10.9622C22.2314 11.5813 22.2314 12.4187 21.7572 13.0378C20.264 14.9868 16.6818 19 12.5 19C8.31818 19 4.73593 14.9868 3.24278 13.0378C2.76851 12.4187 2.76852 11.5813 3.24278 10.9622C4.73593 9.01321 8.31818 5 12.5 5Z"
                                    fill="#979797" />
                                <path
                                    d="M15.5 12C15.5 10.3431 14.1569 9 12.5 9C10.8431 9 9.5 10.3431 9.5 12C9.5 13.6569 10.8431 15 12.5 15C14.1569 15 15.5 13.6569 15.5 12Z"
                    ...
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="#coupon-edit-modal" class="coupon-edit-btn action-btn edit" data-bs-toggle="modal"
                        data-url="{{ route('business.coupons.update', $coupon->id) }}"
                        data-image="{{ asset($coupon->image ?? 'assets/images/icons/upload.png') }}"
                        data-name="{{ $coupon->name }}" data-code="{{ $coupon->code }}"
                    ...
                    @endusercan
                    @usercan('coupon.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.coupons.destroy', $coupon->id) }}"
