    ...
    <div class="erp-table-section">
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                ...
                <form action="{{ route('business.coupons.filter') }}" method="post" class="filter-form" table="#coupons-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.coupons.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.coupons.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.coupons.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('coupon.create')
                        <a type="button" href="#coupon-create-modal"
                            class="add-order-btn rounded-2"
                            class="btn btn-primary" data-bs-toggle="modal"><i
                            class="fas fa-plus-circle me-1"></i>{{ __('Add Coupon') }}
                        </a>
                        @endusercan
            ...
            <div class="card">
                <div class="card-bodys">
                    ...
                    <div class="delete-item delete-show multi-delete-container d-none mb-3">
                        <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                            ...
                            <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal" data-url="{{ route('business.coupons.delete-all') }}">{{ __('Delete') }}</button>
                        </div>
                    </div>
                    @endusercan
                </div>
                ...
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="coupons-data">
                            @include('restaurantwebaddon::coupons.datas')
                        </tbody>
                    </table>
                </div>
                <nav>
                    <ul class="pagination">
                        <li class="page-item">{{ $coupons->links('vendor.pagination.bootstrap-5') }}</li>
                    </ul>
                </nav>
            </div>
        </div>
...
@push('modal')
    ...
    @include('restaurantwebaddon::coupons.create')
    @include('restaurantwebaddon::coupons.edit')
    @include('restaurantwebaddon::coupons.view')
@endpush
