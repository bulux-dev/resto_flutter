    ...
    <div class="erp-table-section">
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                ...
                <form action="{{ route('business.payment-types.filter') }}" method="post" class="filter-form"
                    ...
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.payment-types.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.payment-types.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.payment-types.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('paymentMethod.create')
                            <a type="button" href="#payment-types-create-modal" data-bs-toggle="modal"
                                class="add-order-btn rounded-2"><i class="fas fa-plus-circle me-1"></i>{{ __('Add New') }}</a>
                        @endusercan
                    </div>
                </form>
            </div>
                        ...
                        <div class="delete-item delete-show multi-delete-container d-none mb-3">
                            <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                                ...
                                <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal"
                                    data-url="{{ route('business.payment-types.delete-all') }}">{{ __('Delete') }}</button>
                            </div>
                        </div>
                    @endusercan
                </div>
                ...
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="payment-types-data">
                            @include('restaurantwebaddon::payment-types.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
...
@push('modal')
    ...
    @include('restaurantwebaddon::payment-types.create')
    @include('restaurantwebaddon::payment-types.edit')
@endpush
