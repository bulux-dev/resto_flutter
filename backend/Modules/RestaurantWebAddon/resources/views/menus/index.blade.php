        ...
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                    <span>/ {{ __('Menu List') }}</span>
                </h2>

                <form action="{{ route('business.menus.filter') }}" method="post" class="filter-form" table="#business-menu-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.menus.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.menus.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.menus.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('menus.create')
                        <a type="button" href="#menu-create-modal" data-bs-toggle="modal"
                            class="add-order-btn rounded-2"><i class="fas fa-plus-circle me-1"></i>{{ __('Add Menu') }}</a>
                        @endusercan
                    </div>
                </form>
            </div>
            ...
            <div class="card">
                <div class="card-bodys">
                    ...
                    <div class="delete-item delete-show multi-delete-container d-none mb-3">
                        <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                            ...
                            <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal" data-url="{{ route('business.menus.delete-all') }}">{{ __('Delete') }}</button>
                        </div>
                    </div>
                    @endusercan
                </div>
                ...
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="business-menu-data">
                            @include('restaurantwebaddon::menus.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
...
@push('modal')
    ...
    @include('restaurantwebaddon::menus.create')
    @include('restaurantwebaddon::menus.edit')
@endpush
