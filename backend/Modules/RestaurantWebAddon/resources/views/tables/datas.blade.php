@foreach($tables as $table)
    <tr>
        ...
        <td class="w-60 checkbox">
            <input type="checkbox" name="ids[]" class="delete-checkbox-item multi-delete" value="{{ $table->id }}">
        </td>
        @endusercan
        <td>{{ ($tables->currentPage() - 1) * $tables->perPage() + $loop->iteration }}</td>
        <td class="text-start">{{ $table->name }}</td>
        <td class="text-start">{{ $table->capacity }}</td>
        ...
        <td class="text-center">
            <label class="switch">
                <input type="checkbox" {{ $table->status == 1 ? 'checked' : '' }} class="status"
                    data-url="{{ route('business.tables.status', $table->id) }}">
        ...
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                 ...
                 <a title="{{__('Edit')}}" data-bs-placement="top" href="#tables-edit-modal" data-bs-toggle="modal" class="tables-edit-btn action-btn edit"
                        data-url="{{ route('business.tables.update', $table->id) }}"
                        data-name="{{ $table->name }}"
                        data-capacity="{{ $table->capacity }}"
                        data-status="{{ $table->is_booked }}">
                    ...
                    @endusercan
                    @usercan('tables.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.tables.destroy', $table->id) }}" class="confirm-action action-btn delete" data-method="DELETE">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
