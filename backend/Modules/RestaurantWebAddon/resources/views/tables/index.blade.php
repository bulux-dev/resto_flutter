    ...
    <div class="container-fluid">
        <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
            <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                <span>/ {{ __('Table List') }}</span>
            </h2>

            <form action="{{ route('business.tables.filter') }}" method="post" class="filter-form" table="#tables-data">
                @csrf
                <div class="d-flex align-items-center gap-2 flex-wrap">
                    ...
                    {{-- Filter dropdown --}}
                    <div class="select-wrapper filter-dropdown">
                        <select name="is_booked" class="custom-select">
                    ...
                    <div class="dropdown custom-dropdown">
                        ...
                        <div class="dropdown-content custom-dropdown-content">
                            <a href="{{ route('business.tables.pdf') }}" class="custom-item">
                                {{ __('PDF') }} </a>
                            <a href="{{ route('business.tables.csv') }}" class="custom-item">
                                {{ __('CSV') }} </a>
                            <a href="{{ route('business.tables.excel') }}" class="custom-item">
                                {{ __('EXCEL') }} </a>
                        </div>
                    </div>
                    @usercan('tables.create')
                        <a type="button" href="#tables-create-modal" data-bs-toggle="modal"
                ...
                <div class="delete-item delete-show multi-delete-container d-none mb-3">
                    <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                        ...
                        <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal"
                            data-url="{{ route('business.tables.delete-all') }}">{{ __('Delete') }}</button>
                    </div>
                </div>
                @endusercan
            </div>
                ...
                <table class="table" id="datatable">
                    <thead>
                        <tr>
                            ...
                            <th class="text-start">{{ __('Capacity') }}</th>
                            <th>{{ __('Availability') }}</th>
                            <th>{{ __('Status') }}</th>
                            <th class="text-end">{{ __('Action') }}</th>
                        </tr>
                    </thead>
                    <tbody id="tables-data">
                        @include('restaurantwebaddon::tables.datas')
                    </tbody>
                </table>
            </div>
            <div class="mt-3">
...
@push('modal')
    ...
    @include('restaurantwebaddon::tables.create')
    @include('restaurantwebaddon::tables.edit')
@endpush
