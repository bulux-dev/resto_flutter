@foreach ($expenses as $expense)
    <tr>
        ...
        <td class="text-start">{{ $expense->payment_type->name ?? '' }}</td>
        <td class="text-start">{{ $expense->referenceNo }}</td>
        <td class="text-start">{{ formatted_date($expense->expenseDate) }}</td>
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    @usercan('expense.update')
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="#expenses-edit-modal" data-bs-toggle="modal" class="expense-edit-btn action-btn edit"
                        data-url="{{ route('business.expenses.update', $expense->id) }}"
                        data-expense-category-id="{{ $expense->expense_category_id }}"
                        data-expense-amount="{{ $expense->amount }}" data-expense-for="{{ $expense->expanseFor }}"
                    ...
                    @endusercan
                    @usercan('expense.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.expenses.destroy', $expense->id) }}"
