@foreach ($products as $product)
    <tr>
        ...
        <td><img src="{{ asset($product->images[0] ?? 'assets/img/icon/no-image.svg') }}" alt="Img" class="table-img"></td>

        <td>{{ $product->productName }}</td>
        <td>{{ $product->menu->name ?? '' }}</td>
        <td>{{ $product->category->categoryName ?? '' }}</td>
        <td class="variation-details-btn" data-bs-toggle="modal" data-bs-target="#itemVariationsModal"
            ...
            data-variation='@json($product->variations)'>
        ...
        @else
            <td>{{ currency_format($product->variations?->first()->price, currency: business_currency()) }}</td>
        @endif
        ...
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    ...
                    <a title="{{__('View')}}" data-bs-placement="top" href="#product-view" class="product-view-btn action-btn view" data-bs-toggle="modal"
                        data-view-image="{{ asset($product->images[0] ?? 'assets/img/icon/no-image.svg') }}"
                        data-view-name="{{ $product->productName }}"
                        data-view-category="{{ $product->category->categoryName ?? '' }}"
                        data-view-menu="{{ $product->menu->name ?? '' }}"
                        data-view-food-type="{{ $product->food_type }}"
                        data-view-description="{{ $product->description }}"
                        data-view-preparation-time="{{ $product->preparation_time }}"
                        data-view-variations='@json($product->variations)'
                        data-view-modifier-group='@json($modifierGroups)'>
                    ...
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="{{ route('business.products.edit', $product->id) }}" class="action-btn edit">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                    ...
                    @endusercan
                    @usercan('products.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.products.destroy', $product->id) }}" class="confirm-action action-btn delete"
                        data-method="DELETE">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
