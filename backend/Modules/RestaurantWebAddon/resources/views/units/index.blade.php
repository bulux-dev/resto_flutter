        ...
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                    <span>/ {{ __('Unit List') }}</span>
                </h2>

                <form action="{{ route('business.units.filter') }}" method="post" class="filter-form" table="#units-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.units.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.units.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.units.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('units.create')
                        <a type="button" href="#unit-create-modal" data-bs-toggle="modal" class="add-order-btn rounded-2" class="btn btn-primary"><i class="fas fa-plus-circle me-1"></i>{{ __('Add Unit') }}</a>
                        @endusercan
                    </div>
                </form>
            </div>
            ...
            <div class="card">
                ...
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="units-data">
                            @include('restaurantwebaddon::units.datas')
                        </tbody>
                    </table>
                </div>
                <nav>
                    <ul class="pagination">
                        <li class="page-item">{{ $units->links('vendor.pagination.bootstrap-5') }}</li>
                    </ul>
                </nav>
            </div>
        </div>
...
@push('modal')
    ...
    @include('restaurantwebaddon::units.create')

    @include('restaurantwebaddon::units.edit')
@endpush
