        ...
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                    <span>/ {{ __('Quotation List') }}</span>
                </h2>

                <form action="{{ route('business.quotations.filter') }}" method="post" class="filter-form" table="#quotations-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.quotations.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.quotations.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.quotations.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                    </div>
            ...
            <div class="card">
                    ...
                    @usercan ('quotations.delete')
                        <div class="delete-item delete-show multi-delete-container d-none mb-3">
                            <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                                ...
                                <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal" data-url="{{ route('business.quotations.delete-all') }}">{{ __('Delete') }}</button>
                            </div>
                        </div>
                    @endusercan
                </div>
                ...
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="quotations-data">
                            @include('restaurantwebaddon::quotations.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
