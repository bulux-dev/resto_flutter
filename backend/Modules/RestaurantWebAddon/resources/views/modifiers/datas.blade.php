@foreach ($modifiers as $modifier)
    <tr>
        ...
        <td class="w-60 checkbox">
            <input type="checkbox" name="ids[]" class="delete-checkbox-item  multi-delete" value="{{ $modifier->id }}">
        </td>
        @endusercan
        <td>{{ ($modifiers->currentPage() - 1) * $modifiers->perPage() + $loop->iteration }}</td>
        <td class="text-start">{{ $modifier->product->productName ?? '' }}</td>
        <td class="text-start">{{ $modifier->modifier_group->name ?? '' }}</td>
        <td class="text-start">
            @if ($modifier->is_required == '1')
                <span class="text-danger">{{__('Required')}}</span>
            @else
                <span class="text-secondary">{{__('Optional')}}</span>
            @endif
        </td>
        <td>
        ...
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    ...
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="#modifiers-edit-modal" data-bs-toggle="modal" class="modifiers-edit-btn action-btn edit"
                        data-url="{{ route('business.modifiers.update', $modifier->id) }}"
                        data-product-id="{{ $modifier->product_id ?? '' }}"
                        data-group-id="{{ $modifier->modifier_group_id ?? '' }}"
                        data-required="{{ $modifier->is_required == '1' ? 'checked' : '' }}"
                        data-multiple="{{ $modifier->is_multiple == '1' ? 'checked' : '' }}">
                    ...
                    @usercan('itemModifiers.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.modifiers.destroy', $modifier->id) }}" class="confirm-action action-btn delete"
                        data-method="DELETE">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
