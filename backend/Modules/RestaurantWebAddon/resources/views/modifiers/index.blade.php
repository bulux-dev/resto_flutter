        ...
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                    <span>/ {{ __('Item Modifiers') }}</span>
                </h2>

                <form action="{{ route('business.modifiers.filter') }}" method="post" class="filter-form" table="#modifiers-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        {{-- Filter Dropdown --}}
                        <div class="select-wrapper filter-dropdown">
                            <select name="is_required" class="custom-select">
                        ...
                        <div class="select-wrapper">
                            <select name="is_multiple" class="custom-select">
                                <option value=""> {{ __('Allow Multiple') }} </option>
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.modifiers.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.modifiers.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.modifiers.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('itemModifiers.create')
                        <a type="button" href="#modifiers-create-modal" data-bs-toggle="modal"
                            class="add-order-btn rounded-2"
                            class="btn btn-primary"><i class="fas fa-plus-circle me-1"></i>{{ __('Add Modifier') }}</a>
                        @endusercan
                    </div>
                </form>
            </div>
                ...
                <div class="card-bodys">
                    ...
                    <div class="delete-item delete-show multi-delete-container d-none mb-3">
                        <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                            ...
                            <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal" data-url="{{ route('business.modifiers.delete-all') }}">{{ __('Delete') }}</button>
                        </div>
                    </div>
                    @endusercan
                </div>
                    ...
                    <table class="table" id="datatable">
                        <thead>
                            <tr>
                                ...
                                <th class="text-start">{{ __('Group Name') }}</th>
                                <th class="text-start">{{ __('Required') }}</th>
                                <th>{{ __('Allow Multiple Section') }}</th>
                                <th class="text-end">{{ __('Action') }}</th>
                            </tr>
                        </thead>
                        <tbody id="modifiers-data">
                            @include('restaurantwebaddon::modifiers.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
                    {{ $modifiers->links('vendor.pagination.bootstrap-5') }}
                </div>
            </div>
        </div>
    </div>
...
@push('modal')
    ...
    @include('restaurantwebaddon::modifiers.create')
    @include('restaurantwebaddon::modifiers.edit')
@endpush
