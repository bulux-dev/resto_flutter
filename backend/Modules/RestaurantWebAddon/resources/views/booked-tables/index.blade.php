        ...
        <div class="container-fluid">
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                    <span>/ {{ __('Booked Table') }}</span>
                </h2>

                <form action="{{ route('business.booked-tables.filter') }}" method="post" class="filter-form" table="#booked-tables-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.booked-tables.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.booked-tables.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.booked-tables.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                    </div>
                ...
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        <thead>
                            <tr>
                                <th>{{ __('SL') }}.</th>
                                <th class="text-center">{{ __('Table') }}</th>
                                <th class="text-center">{{ __('Capacity') }}</th>
                                <th>{{ __('Status') }}</th>
                            </tr>
                        </thead>
                        <tbody id="booked-tables-data">
                            @include('restaurantwebaddon::booked-tables.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
                    {{ $booked_tables->links('vendor.pagination.bootstrap-5') }}
                </div>
            </div>
        </div>
    @endsection
