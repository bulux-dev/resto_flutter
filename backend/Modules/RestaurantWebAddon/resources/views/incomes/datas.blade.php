@foreach ($incomes as $income)
    <tr>
        ...
        <td class="text-start">{{ $income->payment_type->name ?? '' }}</td>
        <td class="text-start">{{ $income->referenceNo }}</td>
        <td class="text-start">{{ formatted_date($income->incomeDate) }}</td>
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    @usercan('income.update')
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="#incomes-edit-modal" data-bs-toggle="modal" class="incomes-edit-btn action-btn edit"
                        data-url="{{ route('business.incomes.update', $income->id) }}"
                        data-income-category-id="{{ $income->income_category_id }}"
                        data-income-amount="{{ $income->amount }}" data-income-for="{{ $income->incomeFor }}"
                    ...
                    @endusercan
                    @usercan('income.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.incomes.destroy', $income->id) }}" class="confirm-action action-btn delete"
                        data-method="DELETE">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path fill-rule="evenodd" clip-rule="evenodd"
