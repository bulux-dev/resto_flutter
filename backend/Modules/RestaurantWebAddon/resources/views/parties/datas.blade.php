@foreach ($parties as $party)
    <tr>
        ...
        <td>
            <img src="{{ asset($party->image ?? 'assets/img/icon/no-image.svg') }}" alt="Img"
                class="table-img">
        </td>
        <td>{{ $party->name }}</td>
        <td>{{ $party->email }}</td>
        ...
        <td>{{ $party->phone }}</td>
        @if ($party->due > 0)
        <td class="due-table-text">{{ currency_format($party->due, currency: business_currency()) }}</td>
        @else
        <td class="text-success">{{ currency_format($party->due, currency: business_currency()) }}</td>
        @endif
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    @usercan('parties.view')
                    <a title="{{__('View')}}" data-bs-placement="top" href="#parties-view" class="parties-view-btn action-btn view" data-bs-toggle="modal"
                        data-name="{{ $party->name }}" data-image="{{ asset($party->image ?? 'assets/img/icon/no-image.svg') }}" data-email="{{ $party->email }}"
                        data-phone="{{ $party->phone }}" data-type="{{ ucfirst($party->type) }}"
                        data-due="{{ currency_format($party->due, currency: business_currency()) }}"
                        data-address="{{ $party->address }}" data-delivery-addresses='@json($party->delivery_addresses)'>
                    ...
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="{{ route('business.parties.edit', [$party->id, 'type' => request('type')]) }}" class="action-btn edit">
                        <svg width="25" height="24" viewBox="0 0 25 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                    ...
                    @endusercan
                    @usercan('parties.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.parties.destroy', $party->id) }}"
