        ...
        <div class="container-fluid">
            ...
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                    <span>/ {{ __(':type List', ['type' => __($type)]) }}</span>
                </h2>
                    ...
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.parties.pdf', ['type' => request('type')]) }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.parties.csv', ['type' => request('type')]) }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.parties.excel', ['type' => request('type')]) }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('parties.create')
                            <a type="button" href="{{ route('business.parties.create', ['type' => request('type')]) }}"
                               class="add-order-btn rounded-2">
                                <i class="fas fa-plus-circle me-1"></i>{{ __('Add :type', ['type' => __($type)]) }}
                            </a>
                        @endusercan
                ...
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="parties-data">
                            @include('restaurantwebaddon::parties.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
...
@push('modal')
    ...
    @include('restaurantwebaddon::parties.view')
@endpush
