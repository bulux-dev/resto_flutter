    ...
    <tr>
        ...
        <td class="w-60 checkbox">
            <input type="checkbox" name="ids[]" class="delete-checkbox-item  multi-delete"
                value="{{ $modifier_group->id }}">
        </td>
        @endusercan
        <td>{{ ($modifier_groups->currentPage() - 1) * $modifier_groups->perPage() + $loop->iteration }}</td>
        <td class="text-start">{{ $modifier_group->name }}</td>
        <td class="text-start">{{ Str::words($modifier_group->description, 3, '...' ?? '') }}</td>
        <td class="text-start">
            @foreach ($modifier_group->modifier_group_option as $option)
                <span class=" modifier-color">
                    {{ $option->name }}: {{ currency_format($option->price, currency: business_currency()) }}
                </span>
            @endforeach
        </td>
        <td class="d-print-none">
            <div class="d-flex align-items-center justify-content-end">
                <div class="icon-buttons">
                    ...
                    <a title="{{__('Edit')}}" data-bs-placement="top" href="{{ route('business.modifier-groups.edit', $modifier_group->id) }}" class="action-btn edit">
                    ...
                    @endusercan
                    @usercan('modifierGroups.delete')
                    <a title="{{__('Delete')}}" data-bs-placement="top" href="{{ route('business.modifier-groups.destroy', $modifier_group->id) }}"
