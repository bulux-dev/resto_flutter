        ...
        <div class="container-fluid">
            <div class="section-title mb-3 d-flex align-items-center justify-content-between flex-wrap">
                <h2> <a href="{{ route('business.dashboard.index') }}"> {{ __('Dashboard') }} </a>
                    <span>/ {{ __('Modifier Groups') }}</span>
                </h2>

                <form action="{{ route('business.modifier-groups.filter') }}" method="post" class="filter-form" table="#modifier-groups-data">
                    @csrf
                    <div class="d-flex align-items-center gap-2 flex-wrap">
                        ...
                        <div class="dropdown custom-dropdown">
                            ...
                            <div class="dropdown-content custom-dropdown-content">
                                <a href="{{ route('business.modifier-groups.pdf') }}" class="custom-item">
                                    {{ __('PDF') }} </a>
                                <a href="{{ route('business.modifier-groups.csv') }}" class="custom-item">
                                    {{ __('CSV') }} </a>
                                <a href="{{ route('business.modifier-groups.excel') }}" class="custom-item">
                                    {{ __('EXCEL') }} </a>
                            </div>
                        </div>
                        @usercan('modifierGroups.create')
                            <a type="button" href="{{ route('business.modifier-groups.create') }}"
                                class="add-order-btn rounded-2"
                                class="btn btn-primary"><i class="fas fa-plus-circle me-1"></i>{{ __('Add Modifier Group') }}</a>
                        @endusercan
                    </div>
                </form>
            </div>
            ...
            <div class="card ">
                <div class="card-bodys">
                    ...
                    <div class="delete-item delete-show multi-delete-container d-none mb-3">
                        <div class="delete-item-show d-flex align-items-center justify-content-between w-100">
                            ...
                            <button data-bs-toggle="modal" class="trigger-modal" data-bs-target="#multi-delete-modal" data-url="{{ route('business.modifier-groups.delete-all') }}">{{ __('Delete') }}</button>
                        </div>
                    </div>
                    @endusercan
                </div>
                ...
                <div class="responsive-table m-0">
                    <table class="table" id="datatable">
                        ...
                        <tbody id="modifier-groups-data">
                            @include('restaurantwebaddon::modifier-groups.datas')
                        </tbody>
                    </table>
                </div>
                <div class="mt-3">
                    {{ $modifier_groups->links('vendor.pagination.bootstrap-5') }}
                </div>
            </div>
        </div>
    </div>
