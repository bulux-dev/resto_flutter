            ...
            @if (Request::get('type') == 'sales')
                @foreach ($sales as $sale)
                    <tr>
                        ...
                        @foreach ($vats as $vat)
                            <td>{{ $sale->vat_id == $vat->id ? currency_format($sale->vat_amount, currency: business_currency()) : '0' }}
                            </td>
                        @endforeach
                    </tr>
                @endforeach
            ...
            @if (Request::get('type') == 'purchases')
                @foreach ($purchases as $purchase)
                    <tr>
                        ...
                        @foreach ($vats as $vat)
                            <td>{{ $purchase->vat_id == $vat->id ? currency_format($purchase->vat_amount, currency: business_currency()) : '0' }}
                            </td>
                        @endforeach
                    </tr>
                @endforeach
