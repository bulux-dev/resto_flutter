...
<div class="sales-containers">
    <div id="salesContent" class="sales-content">
        <div id="salesRight" class="sales-right-content">
            <form action="{{ route('business.sales.product-filter') }}" method="post" class="product-filter product-filter-form w-100" table="#products-list">
                @csrf
                  <div id="searchContainer" class="search-container custom-padding">
                        <i class="search-icon">
                    ...
                    <div class="category-btn-container custom-padding">
        ...
        <div id="salesLeft" class="left-content-sales">
            <form action="{{ route('business.sales.store') }}" method="post" enctype="multipart/form-data" class="ajaxform_redirect_invoice" data-default_route="{{ route('business.sales.store') }}">
                @csrf
            <div class="sales-left-content">
                <div class="button-group">
                    <button type="button" class="tab-btn saleType active-sale-type" data-type="dine_in">{{__('Dine In')}}</button>
                    <button type="button" class="tab-btn saleType" data-type="pick_up">{{__('Pickup')}}</button>
                    <button type="button" class="tab-btn saleType" data-type="delivery">{{__('Delivery')}}</button>
                    <button type="button" class="tab-btn saleType" data-type="quotation" data-quotation_route={{ route('business.quotations.store') }} >{{__('Order Quotation')}}</button>
                </div>
                <div class="order-form-section select-container">
                    <!-- for dine in-->
                    <div class="row g-3" data-type="dine_in">
                        <div class="col-md-6">
                            <div class="gpt-up-down-arrow position-relative">
                                     <select name="staff_id" class="form-control select-waiter table-select w-100 ">
                         ...
                         <div class="col-md-6">
                            <div class="custom-dropdown-2 gpt-up-down-arrow">

                                <div class="dropdown-toggle">{{__('Select Table')}}</div>
                                <div class="dropdown-menu">
                                    @foreach ($tables as $table)
                                    <div class="dropdown-item tableSelect {{ $table->is_booked == 1 ? 'booked' : ''  }}" data-id="{{ $table->id }}">
                                        {{ $table->name }}
                                        @if ($table->is_booked == 1)
                                        <div class="hold">{{__('Hold')}}</div>
                                        @endif
                                    </div>
                                    @endforeach
                                </div>
                         ...
                         <input type="hidden" name="sales_type" value="dine_in">
                         <div class="col-md-12">
                             <div class="">
                                 <div class="gpt-up-down-arrow position-relative custom-select-wrapper">
                                  <select name="party_id" class="form-control border-0 table-select customer-select w-100 choices-select" data-modal="#customer-create-modal">
                                        <option value="">{{__('Select Customer')}}</option>
                                        @foreach ($customers as $cstomer)
                                            <option value="{{ $cstomer->id }}">
                                                {{ $cstomer->name }}({{__('Due')}}: {{ currency_format($cstomer->due ?? 0, currency: business_currency()) }})
                                            </option>
                                        @endforeach
                                    </select>
                                     <span></span>
                    ...
                    {{-- for delivery --}}
                    <div class="row g-3 mt-3" data-type="delivery">
                        <div class="col-md-6 m-0">
                            <input type="number" name="delivery_charge" required class="form-control deliveryCharge" placeholder="{{ __('Enter delivery charge') }}">
                         </div>

                         <input type="hidden" name="sales_type" value="delivery">

                         <div class="col-md-6 m-0">
                            <div class="gpt-up-down-arrow position-relative">
                                     <select name="party_id" id="customerSelect" class="form-control table-select w-100 getCustomerId choices-select" data-modal="#customer-create-modal">
                                     <option value="">{{__('Select Customer')}}</option>
                                     @foreach ($customers as $cstomer)
                                     <option value="{{ $cstomer->id }}">{{ $cstomer->name }}({{__('Due')}}: {{ currency_format($cstomer->due ?? 0, currency: business_currency()) }})</option>
                                     @endforeach
                                 </select>
                                 <span></span>
                             </div>
                         ...
                         <div class="col-md-12">
                             <div class="">
                                 <div class="gpt-up-down-arrow position-relative custom-select-wrapper">
                                     <select name="address_id" id="deliveryAddressAppend" class="form-control table-select customer-select w-100">
                                         <option value="">{{__('Select Address')}}</option>
                                            {{-- dynamic delivery address come here --}}
                                     </select>
                                     <span></span> <!-- empty span thakbe -->
                                     @usercan('parties.create')
                                     <div class="custom-add-button">
                                         <i class="deliveryAddBtn" id="openDeliveryAddressModal">
                                             <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                             <path d="M10.0009 4.16675V15.8351" stroke="#FC8019" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                             <path d="M15.835 10.0017H4.16663" stroke="#FC8019" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                                             </svg>
                    ...
                    {{-- for quotation --}}
                    <div class="row g-3 mt-3" data-type="quotation">
                        <input type="hidden" name="sales_type" value="quotation">
                        <div class="col-md-12">
                            <div class="">
                                <div class="gpt-up-down-arrow position-relative custom-select-wrapper">
                    ...
                    <input type="hidden" name="sales_type" id="salesTypeForPickUp" disabled>
                </div>
                    ...
                    <div class="bottom-amount-section">
                         <div class="payment-container">
                                ...
                                <div class="custom-form-group">
                                 ...
                                 <input type="number" name="paidAmount" id="receive_amount" class="custom-input" placeholder="{{__('Ex: 10')}}">
                                </div>
                                ...
                                <div class="custom-form-group">
                                    <p>{{__('Due Amount')}}</p>
                                <input type="number" name="dueAmount" id="due_amount" class="custom-input " placeholder="{{__('Ex: 10')}}" readonly>
                                </div>

                                <div class="bottom-button-group">
                                <button class="bottom-button cancel cancel-sale-btn" data-route="{{ route('business.sale-carts.remove-all') }}">{{__('Cancel')}}</button>
                                <button type="submit" class="bottom-button print submit-btn" name="is_print" value="1">{{__('Print')}}</button>
                                <button type="submit" class="bottom-button kot submit-btn" name="is_kot" value="1">{{__('KOT')}}</button>
                                </div>
                            </div>
                            ...
                            <div class="payment-container">
                                ...
                                <div class="pay-row">
                                    <span class="pay-label-text vatOnSale" data-vat-sale="{{ $vat_on_sale->rate ?? 0 }}">{{ $vat_on_sale->name ?? 'N/A' }} ({{ $vat_on_sale->rate ?? 0 }}%)</span>
                                ...
                                <div class="pay-row coupon-row">
                                    <div class="d-flex align-items-center">
                                        <a href="#couponModal" class="pay-label-text coupon getCoupon" data-bs-toggle="modal">{{__('Coupon')}}<span class="text-black couponPercentageShow"></span></a>
                                ...
                                <input type="hidden" class="coupondiscount" id="coupondiscount">
                                <input type="hidden" class="couponType" id="couponType">


                                <div class="pay-row discount-row">
                                <span class="pay-label-text">{{__('Discount')}} (<span class="discountPercentageShow"></span>%)</span>
                                <input type="number" name="discountAmount" class="discount-input discount_amount" placeholder="{{__('Ex: 10')}}">
                                </div>
                                <input type="hidden" name="discountPercentage" class="discountPercentageShow">

                                <div class="pay-row discount-row">
                                <span class="pay-label-text">{{__('Tips')}}</span>
                                <input type="number" name="tip" class="discount-input tip" placeholder="{{__('Ex: 10')}}">
                                </div>

                                @usercan ('sales.create')
                                <button type="submit" class="payment-footer w-100 submit-btn" id="paymentBtnHide" name="is_payment" value="1">
                                    <div class="badge-box totalCartProduct">0</div>
                                    <p  class="footer-label">{{__('Payment')}}:</p>
                                    <span class="pay-amount totalAmmount"></span>
                                </button>
                                <button class="payment-footer w-100" id="saveBtn">
                                    <div class="badge-box totalCartProduct">0</div>
                                    <p type="submit" class="footer-label submit-btn">{{__('Save')}}</p>
                                    <span class="pay-amount totalAmmount"></span>
                                </button>
                                @endusercan
                                <input type="hidden" name="totalAmount" class="totalAmmount">
                            </div>
                    </div>
               </div>
            </form>
...
<input type="hidden" value="{{ route('business.sales.getDeliveryAddress') }}" id="getDeliveryAddress">
...
@push('modal')
@include('restaurantwebaddon::sales.delivery-address')
...
@push('js')
    ...
    <script src="{{ asset('assets/js/custom/responsive2.js') }}"></script>
@endpush
