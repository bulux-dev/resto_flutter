            ...
            <div id="salesLeft2" class="left-content-sales">
                ...
                <div class="sales-left-content">
                    <div class="order-form-section select-container">
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                <div class="gpt-up-down-arrow position-relative">
                                         ...
                                         <option @selected($sale->staff_id == $staff->id) value="{{ $staff->id }}">{{ $staff->name }}</option>
                                         @endforeach
                                     </select>
                                     <span></span>
                                 </div>
                             ...
                             <div class="col-md-12">
                                 <div class="">
                                     <div class="gpt-up-down-arrow position-relative custom-select-wrapper">
                                         <select name="party_id" id="customer_id" class="form-control table-select customer-select w-100 choices-select" data-modal="#customer-create-modal">
                                             <option value="">{{__('Select Customer')}}</option>
                                             @foreach ($customers as $cstomer)
                                             <option @selected($sale->party_id == $cstomer->id) value="{{ $cstomer->id }}">{{ $cstomer->name }}({{__('Due')}}: {{ currency_format($cstomer->due ?? 0, currency: business_currency()) }})</option>
                                             @endforeach
                                         </select>
                        ...
                        {{-- for delivery --}}
                        @if ($sale->sales_type == 'delivery')
                        <div class="row g-3 mt-3">
                            <div class="col-md-6 m-0">
                                <input type="number" name="delivery_charge" value="{{ $sale->meta['delivery_charge'] ?? '' }}" class="form-control deliveryCharge" placeholder="{{ __('Enter delivery charge') }}">
                             </div>
                             ...
                             <input type="hidden" id="selectedDeliveryAddressId" value="{{ $sale->address_id }}">
                             <div class="col-md-12">
                                 <div class="">
                                     <div class="gpt-up-down-arrow position-relative custom-select-wrapper">
                        ...
                        <input type="hidden" name="sales_type" id="salesTypeForPickUp">
                        @endif
                    </div>
                        ...
                        <div class="bottom-amount-section">
                             <div class="payment-container">
                                    ...
                                    <select  name="payment_type_id" id="payment-type" class="custom-input">
                                        ...
                                        <option @selected($sale->payment_type_id == $payment_type->id) value="{{ $payment_type->id }}">{{ $payment_type->name }}</option>
                                        @endforeach
                                    </select>
                                    </div>
                                    ...
                                    <input type="number" name="paidAmount" value="{{ $sale->paidAmount }}" id="receive_amount" class="custom-input" placeholder="{{__('Ex: 10')}}">
                                    </div>
                                ...
                                <div class="payment-container">
                                    ...
                                    <input type="hidden" class="coupondiscount" value="{{ $sale->coupon->discount ?? '' }}" id="coupondiscount">
                                    <input type="hidden" class="couponType" value="{{ $sale->coupon->discount_type ?? '' }}" id="couponType">


                                    <div class="pay-row discount-row">
                                    <span class="pay-label-text">{{__('Discount')}} (<span class="discountPercentageShow"></span> %)</span>
                                    <input type="number" name="discountAmount" value="{{ $sale->discountAmount }}" class="discount-input discount_amount" placeholder="{{__('Ex: 10')}}">
                                    </div>
                                    <input type="hidden" name="discountPercentage" class="discountPercentageShow">

                                    <div class="pay-row discount-row">
                                    <span class="pay-label-text">{{__('Tips')}}</span>
                                    <input type="number" name="tip" value="{{ $sale->meta['tip'] ?? '' }}" class="discount-input tip" placeholder="{{__('Ex: 10')}}">
                                    </div>
                                </div>
                        </div>
