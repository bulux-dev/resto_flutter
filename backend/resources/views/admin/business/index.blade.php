                        ...
                        <form action="{{ route('admin.business.filter') }}" method="post" class="filter-form"
                            ...
                            <div class="table-top-left d-flex gap-3 ">
                                ...
                                <div class="table-search position-relative">
                                    <input class="form-control searchInput header-input" type="text" name="search"
...
<script>
     document.addEventListener("DOMContentLoaded", function () {
        const checkboxes = document.querySelectorAll('.multi-delete');

        checkboxes.forEach(function (checkbox) {
            checkbox.addEventListener('change', function () {
                const label = this.closest('.table-custom-checkbox');
                if (this.checked) {
                    label.classList.add('checkbox-checked-red');
                } else {
                    label.classList.remove('checkbox-checked-red');
                }
            });
        });
    });
